...
public class Day16_2_GraphApproachCached {
    ...
    protected void count() throws Exception {
        ...
        long currentTile = 0;
        int totalFound = 0;
        for (Node tile : visitedSortedList) {

            if (currentTile % 1000 == 0) {
                System.out.println("processing " + currentTile + " of " + totalTiles + ", found " + totalFound);
            }
            ...
            if (minScoreS != null && minScoreE != null) {
                ...
                if (Math.abs(totalScore - minScore) <= 1001) {
                //if (totalScore == minScore) {
                    scoreBoard.putIfAbsent(totalScore, new HashSet<>());
                    scoreBoard.get(totalScore).add(new Tile(tile));
                    totalFound=scoreBoard.get(totalScore).size();
                }
            }
            currentTile++;
        }
        //System.out.println("answer = " + ( scoreBoard.get(minScore).size()));
        //printTilesMap(scoreBoard.get(minScore));
        List<Long> sortedList = new ArrayList<>(scoreBoard.keySet());
        Collections.sort(sortedList);
        int top = 0;
        for (long key : sortedList) {
            System.out.println("answer %d of score %d = %d".formatted(top, key, scoreBoard.get(key).size()));
            if (top > 5) {
                break;
            }
        }
    ...
    protected Long bsfMinScore(Node start, Node target, boolean matchDirection, Map<Node, Long> cache, boolean useCache) {
        ...
        while (!queue.isEmpty()) {
            Node current = queue.poll();
            if(useCache) {
                //System.out.println("Examine : " + current);
                if (cache.get(current) != null) {
                    Long retVal = current.score + cache.get(current);
                    result.add(retVal);
                    visited.add(current);
                    continue;
                    /*System.out.println("Cache PUT : " + start + ", score = " + retVal);
                    cache.put(start, retVal);
                    return retVal;*/
                } else {
                    if (current.x == target.x && current.y == target.y) {
                        if (matchDirection) {
        ...
        Long retVal = result.stream().min(Comparator.comparingLong(Long::longValue)).orElse(null);
        if(retVal != null) {
            if(useCache) {
                //System.out.println("Cache ADD : " + start + ", score = " + retVal);
                cache.put(start, retVal);
            }
        }
        return retVal;
