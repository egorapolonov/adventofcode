...
public class Day16_2_GraphApproach {
    ...
    public static void main(String[] args) throws Exception {
        new Day16_2_GraphApproach().count();
    ...
    protected void count() throws Exception {
        ...
        /*Long minSumScore = scoreBoard.keySet().stream().mapToLong(Long::longValue).min().orElseThrow();
        System.out.println("answer = " + ( scoreBoard.get(minSumScore).size() + 1));
        List<Long> sortedList = new ArrayList<>(scoreBoard.keySet());
        Collections.sort(sortedList);
        int top = 0;
        for (long key : sortedList) {
            System.out.println("answer %d of score %d = %d".formatted(top, key, scoreBoard.get(minSumScore).size()));
            if (top > 5) {
                break;
            }
        }*/
